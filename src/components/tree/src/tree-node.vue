<template>
  ...
  >
    ...
    <el-collapse-transition>
      <div class="el-tree-node__children" :class="{ 'btn-content': node.data.buttonList && node.data.buttonList.length > 0 && node.data.childList && node.data.childList.length > node.data.buttonList.length, 'no-right-line': !node.data.childList || node.data.childList.length < 1 }" v-if="!renderAfterExpand || childNodeRendered" v-show="expanded" role="group" :aria-expanded="expanded">
