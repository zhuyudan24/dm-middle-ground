<template>
  <div class="select-shop-wrap">
    ...
    <p class="wxy-tip">按观云台门店分组，应用于万象仪</p>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    getGroupTreeData() {
      this.axios
        .get('/api-admin/cloud-store-group', {
          params: {
    ...
    getSearchData(val) {
      ...
      if (val) {
        this.axios
          .post(
            '/api-admin/cloud-store',
            qs.stringify({
              requestProject: 'middle-ground',
              search: this.searchValueList,
              currentPage: 1,
              pageSize: 5,
          ...
          .then(res => {
            ...
            // log(data);
            if (data.errorCode == 0) {
              this.selectList = data.result.list ? data.result.list : [];
            } else {
              this.$message.error(data.message);
            }
          });
